...
export class UserService {
  ...
  private convertCollection(users: UserResponse[], langCode: ELang) {
    return users.map((user) => ({
      ...
      address:
        'address' in user
          ? this.userHelper.convertAddress<UserAddress>(
  ...
  async getUsers(query: QueryDto) {
    const { page, limit, langCode, keywords, sortBy, gender, role, staffOnly } =
      query;
    ...
    const users = await this.prisma.user.findMany({
      where: {
        AND: [
          ...
          {
            role: staffOnly
              ? { not: ERole.USER, in: role ? [Number(role)] : undefined }
              : role && Number(role),
          },
  ...
  async createUser(query: QueryDto, file: Express.Multer.File, user: UserDto) {
    ...
    if (newUser) {
      ...
      if (address) {
        ...
        const { addressEn, addressVn, cityCode, districtCode, wardCode } =
          addressJson;
      ...
      if (file) {
        ...
        const image = utils.generateFile(result, { userId: newUser.id });
  ...
  async updateUser(query: QueryDto, file: Express.Multer.File, user: UserDto) {
    const { userId, langCode } = query;
    const { role, firstName, lastName, phone, gender, birthday, address } =
      user;
    ...
    if (file) {
      ...
      const result = await this.cloudinary.upload(utils.getFileUrl(file));
      const image = utils.generateFile(result, { userId });
      if (updateUser.image) {
